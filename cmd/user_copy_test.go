...
func TestCommandHelpHasNoInternalIssueReferences(t *testing.T) {
	ref := regexp.MustCompile(`(?i)(?:aceteam|citadel(?:-cli)?)\s*#\d+|(?:issue|PR|see|before)\s+#\d+|\(#\d+\)`)
...
func TestIdentityOutputHasNoInternalIssueReferences(t *testing.T) {
	...
	if regexp.MustCompile(`(?i)(?:aceteam|citadel(?:-cli)?)\s*#\d+`).MatchString(output.String()) {
...
func TestLocalToolDescriptionsHaveNoInternalIssueReferences(t *testing.T) {
	ref := regexp.MustCompile(`(?i)(?:aceteam|citadel(?:-cli)?)\s*#\d+|\(#\d+\)`)
