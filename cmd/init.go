...
func getNodeName() (string, error) {
	...
	// Offer a friendlier name when the resolved name is auto-generated.
	return offerFriendlierName(generated), nil
}

// offerFriendlierName interactively offers to replace an auto-generated
// "citadel-<id>" hostname with a friendlier user-chosen name during
// registration. It is a no-op (returns the input unchanged) when:
//   - the name is not auto-generated,
//   - no interactive terminal is attached,
//   - an explicit --node-name flag was provided, or
//   - the user declines or enters an invalid name.
func offerFriendlierName(name string) string {
	if initNodeName != "" || !platform.IsAutoGeneratedHostname(name) || !tui.IsTTY() {
		return name
	}

	fmt.Printf("\nThis node will register as the auto-generated name '%s'.\n", name)
	answer, err := ui.AskInput(
		"Enter a friendlier name (or leave blank to keep the generated one)",
		name,
		name,
	)
	if err != nil {
		Debug("friendlier-name prompt skipped: %v", err)
		return name
	}
	...
	if !platform.IsValidHostname(answer) {
		fmt.Printf("⚠️  '%s' is not a valid hostname; keeping '%s'.\n", answer, name)
		return name
	}
	return answer
}
...
// getOriginalHostname reads the pre-registration OS hostname previously stored
// in the global config file. Returns an empty string if none was recorded.
func getOriginalHostname() string {
	globalConfigFile := filepath.Join(platform.ConfigDir(), "config.yaml")
	data, err := os.ReadFile(globalConfigFile)
	if err != nil {
		return ""
	...
	var config struct {
		OriginalHostname string `yaml:"original_hostname"`
	}
	if err := yaml.Unmarshal(data, &config); err != nil {
		return ""
	}
	return config.OriginalHostname
}

// saveOriginalHostnameToConfig records the original pre-registration OS hostname
// in the global config file so it can be restored later. It only writes the
// value the first time (when no original hostname has been recorded yet), so the
// genuinely original name is preserved across repeated registrations.
func saveOriginalHostnameToConfig(hostname string) error {
	if hostname == "" {
		return nil
	}
	...
	config["original_hostname"] = hostname
...
func connectToNetwork(nodeName, authKey string) error {
	// Record the pre-registration OS hostname (once) so it can be restored
	// later via 'citadel rename'. saveOriginalHostnameToConfig is a no-op if an
	// original hostname has already been recorded.
	if current, err := os.Hostname(); err == nil && current != "" {
		if saveErr := saveOriginalHostnameToConfig(current); saveErr != nil {
			Debug("warning: could not save original hostname to config: %v", saveErr)
		}
	}
