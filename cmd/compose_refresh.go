...
// aceteam-ai/citadel-cli#1060 (extraction/diffusers/transcribe/lmstudio). These
...
// prefix, not a driftable 0.0.0.0 default). lmstudio's pinned image does not
// exist on Docker Hub (aceteam-ai/citadel-cli#1066), so no container can ever be
// running to recreate -- its entry here is inert but kept for consistency until
// #1066 fixes or retires the engine. TestLoopbackDriftEnginesPublishLoopback pins
// this set against the actual compose templates so it cannot silently disagree
// with what #1025/#1060 edited.
