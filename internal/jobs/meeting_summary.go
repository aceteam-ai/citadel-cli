...
// As of citadel-cli#1076 services.VLLMHostPort itself resolves from the
// CITADEL_VLLM_HOST_PORT env var at process init (defaulting to 8201), so a
// per-node host-port override IS honored here — the same value the vLLM compose
// publish resolves to.
